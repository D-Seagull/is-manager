...
export function ensureSocketAlive(): void {
  ...
  // Connected flag is true but no packet (not even a ping) has arrived for a
  // while → the transport is a zombie. Cycle it.
...
export function getSocket(token?: string): Socket {
  if (!socket) {
    ...
    // engine 'upgrade' isn't in the Manager's typed event map — cast to log it.

...
// otherwise the login screen would fall back to the device locale every time
// the user record is absent.
...
/**
 * Apply a language chosen in Settings (or restored from the user record) and
 * remember it. Called with a falsy value (e.g. after logout) it is a no-op, so
 * the previously chosen language stays in effect.
 */

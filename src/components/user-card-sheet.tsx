...
import { useUserDetail } from '@/hooks/use-people';
...
/**
 * A read-only mini profile — opened by tapping a sender's name/avatar in chat.
 * Shows who someone is (name, role, contacts, language, team lead) without the
 * full-screen profile's actions. Available to every user.
 */
export function UserCardSheet({
  ...
  const { data: person, isLoading } = useUserDetail(userId);
          ...
          {isLoading || !person ? (
            ...
            <>
              <View style={styles.profile}>
                <View>
                  ...
                  <View style={styles.dotWrap}>
                    <StatusDot user={person} size={13} ring={c.card} />
                  </View>
                </View>
              ...
              <View style={styles.info}>
                ...
                {person.currentTruck ? (
                  <Row
                    iconNode={<MaterialCommunityIcons name="truck-outline" size={16} color={c.mutedForeground} />}
                    label={t('nav.items.trucks', 'Вантажівка')}
                    value={person.currentTruck.plate}
                    c={c}
                  />
                ) : null}
                {person.teamlead ? (
                  <Row
                    icon="ribbon-outline"
                    label={t('people.teamlead', 'Тімлід')}

...
// Shape returned by GET /groups/managers — raw Group rows with creator +
// managers includes (see groups.service.findAllManagersGroups).
...
// One entry of GET /group-messages/unread — per-group unread count for the
// current user's non-DRIVERS groups. Used to badge the manager Groups list.
...
// GET /group-messages/unread returns { total, items } — total is the
// pre-summed unread across all the user's groups.
...
/**
 * Per-group unread counts for the current manager's groups (non-DRIVERS).
 * Backs the unread badges on the Groups tab + list rows.
 */
export function useGroupUnread() {
  return useQuery<GroupUnreadSummary>({
    queryKey: groupKeys.unread,
    queryFn: async () => {
